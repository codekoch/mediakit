...
zenity --width=400 --height=100 --info --text="Internet für alle verbundenen Geräte gesperrt!" --timeout=2 2> /dev/null  &
else 
blockip=$1 
sudo iptables -I FORWARD -s $1 -j DROP
zenity --width=400 --height=100 --info --text="Internet für $1 gesperrt!" --timeout=2 2> /dev/null  &
fi
